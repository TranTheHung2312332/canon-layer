...
Dependencies:
    pip install -U "google-genai[aiohttp]" python-dotenv tqdm
...
# Kept byte-for-byte compatible with v1.2 so existing request hashes/checkpoints
# remain reusable. Clean up this duplicated sentence only when intentionally
# starting a new prompt version.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=4,
        help=(
            "Maximum number of simultaneous Gemini API requests. Start with 4; "
            "lower it if the API returns frequent 429 errors."
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=600.0,
        help="Timeout for one API attempt; retries are handled separately.",
    )
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Optional pause after a successful request. Bounded concurrency normally "
            "makes an additional pause unnecessary."
        ),
    )
    parser.add_argument(
        "--max-files",
...
def build_chunks(
    *,
    ...
    for paragraph in paragraphs:
        ...
        # The rendered form is "[id] text". Newlines do not affect split(), so
        # summing each rendered paragraph is exactly equivalent to repeatedly
        # rendering the entire current group, but avoids quadratic work.
        current_tokens += whitespace_token_count(
...
def exception_status_code(exc: Exception) -> int | None:
    """Extract an HTTP status from SDK exceptions across google-genai versions."""
    ...
    match = re.search(r"\b(408|409|425|429|500|502|503|504)\b", str(exc))
...
async def generate_text_with_retry(
    *,
    ...
    """Generate visible model text with bounded concurrency and transient retries.

    The semaphore covers only the network request. Backoff and optional pacing happen
    outside it, so one throttled request cannot block all other work.
    """
    ...
    for attempt in range(settings.max_retries + 1):
        try:
            ...
            transient_transport_error = isinstance(
                exc,
            ...
            retryable = (
                ...
                else status in retryable_statuses or transient_transport_error
            )
...
async def summarize_chunk(
    *,
    ...
    summary = await generate_text_with_retry(
        client=client,
        settings=settings,
        semaphore=semaphore,
        prompt=prompt,
...
async def process_novel_summaries(
    *,
    client: Any,
    settings: ApiSettings,
    semaphore: asyncio.Semaphore,
    ...
    chunk_checkpoint_dir: Path,
    novel_checkpoint_dir: Path,
    chunk_summary_min_words: int,
    chunk_summary_max_words: int,
    novel_summary_min_words: int,
    novel_summary_max_words: int,
    chunk_max_output_tokens: int,
    novel_max_output_tokens: int,
    progress: tqdm,
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """Summarize one novel while allowing its independent chunks to run in parallel."""

    async def run_chunk(chunk: Chunk) -> dict[str, Any]:
        result = await summarize_chunk(
            ...
            min_words=chunk_summary_min_words,
            max_words=chunk_summary_max_words,
            max_output_tokens=chunk_max_output_tokens,
        )
        progress.update(1)
        return result

    chunk_tasks = [asyncio.create_task(run_chunk(chunk)) for chunk in chunks]
    try:
    ...
    # gather() preserves input order, but sort again to make the dependency explicit
    # and protect against future orchestration changes.
    chunk_summaries.sort(key=lambda row: int(row["chunk_index"]))

    novel_summary = await summarize_novel(
        client=client,
        settings=settings,
        semaphore=semaphore,
        novel_id=novel_id,
        ...
        min_words=novel_summary_min_words,
        max_words=novel_summary_max_words,
        max_output_tokens=novel_max_output_tokens,
    )
    progress.update(1)
    return chunk_summaries, novel_summary
...
async def generate_all_summaries(
    *,
    api_key: str,
    settings: ApiSettings,
    prepared: Sequence[tuple[Path, int, str, list[Paragraph], list[Chunk]]],
    chunk_checkpoint_dir: Path,
    novel_checkpoint_dir: Path,
    chunk_summary_min_words: int,
    chunk_summary_max_words: int,
    ...
    """Run all level-1 and level-2 generations using one shared async client."""
    try:
    ...
    root_client = genai.Client(api_key=api_key)
    async_client = root_client.aio
    semaphore = asyncio.Semaphore(settings.max_concurrency)
    total_requests = sum(len(chunks) + 1 for _, _, _, _, chunks in prepared)
    progress = tqdm(total=total_requests, desc="API summaries", unit="summary")

    tasks = [
        asyncio.create_task(
            process_novel_summaries(
                client=async_client,
                settings=settings,
                semaphore=semaphore,
                path=path,
                ...
                title=title,
                chunks=chunks,
                chunk_checkpoint_dir=chunk_checkpoint_dir,
                novel_checkpoint_dir=novel_checkpoint_dir,
                chunk_summary_min_words=chunk_summary_min_words,
                chunk_summary_max_words=chunk_summary_max_words,
                novel_summary_min_words=novel_summary_min_words,
                novel_summary_max_words=novel_summary_max_words,
                chunk_max_output_tokens=chunk_max_output_tokens,
                novel_max_output_tokens=novel_max_output_tokens,
                progress=progress,
            )
        )
        for path, novel_id, title, _, chunks in prepared
    ]
...
def main() -> None:
    ...
    if args.request_pause_seconds < 0:
        raise ValueError("--request-pause-seconds cannot be negative")
    ...
    # Build deterministic non-model rows before starting network work. This also
    # performs the global paragraph-to-chunk uniqueness check before paid requests.
    raw_chunk_rows: list[dict[str, Any]] = []
    paragraph_mapping_rows: list[dict[str, Any]] = []
    paragraph_to_chunk: dict[str, str] = {}

    for _, novel_id_value, _, _, chunks in prepared:
        for chunk in chunks:
    ...
    LOG.info(
        "Starting async generation: concurrency=%d, timeout=%.1fs, pause=%.2fs",
    ...
    generated = asyncio.run(
        generate_all_summaries(
            ...
            chunk_checkpoint_dir=chunk_checkpoint_dir,
            novel_checkpoint_dir=novel_checkpoint_dir,
            chunk_summary_min_words=args.chunk_summary_min_words,
            chunk_summary_max_words=args.chunk_summary_max_words,
            novel_summary_min_words=args.novel_summary_min_words,
            novel_summary_max_words=args.novel_summary_max_words,
            chunk_max_output_tokens=args.chunk_max_output_tokens,
            novel_max_output_tokens=args.novel_max_output_tokens,
        )
    )

    chunk_summary_rows = [
        row
        for per_novel_chunk_summaries, _ in generated
        for row in per_novel_chunk_summaries
    ]
    novel_summary_rows = [novel_summary for _, novel_summary in generated]
    chunk_to_novel_summary = {
        str(chunk_summary["_id"]): str(novel_summary["_id"])
        for chunk_summaries, novel_summary in generated
